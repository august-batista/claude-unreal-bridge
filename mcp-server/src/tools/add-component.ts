...
export function registerAddComponentTool(server: McpServer): void {
  server.registerTool(
    "add-component",
    {
      title: "Add Component",
      description:
        "Add a component to a Blueprint's component tree (the Components panel), then compile and save. " +
      ...
      inputSchema: {
        ...
        componentClass: z
          .string()
          .describe(
            "Component class: a UE Python name (\"StaticMeshComponent\", \"SpringArmComponent\", \"CameraComponent\", " +
    ...
    async ({ projectPath, blueprintPath, componentClass, componentName, parentComponent, compile }, extra) => {
      try {
        ...
        const data = result.data as AddComponentResult;
        if (!data.success) {
          return {
            isError: true,
            content: [{ type: "text" as const, text: `Add component failed: ${data.error ?? "unknown error"}` }],
          };
        }
