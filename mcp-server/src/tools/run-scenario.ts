...
export function registerRunScenarioTool(server: McpServer): void {
  server.tool(
    "run-scenario",
    "Boot a map headlessly and run console commands against it, then capture filtered logs from the run. " +
      ...
      "End your `execCmds` with `Quit` for a clean exit; otherwise rely on `timeoutMs`.",
    {
      ...
      mapPath: z
        .string()
        .describe(
          "Map to boot. Accepts asset paths (`/Game/Maps/MyMap`) or short names (`MyMap`).",
        ),
      execCmds: z
        .array(z.string())
        .default([])
        .describe(
          "Console commands to run after the map loads, in order. Example: [\"showdebug game\", \"ke * StartScenario\", \"Quit\"]. Include `Quit` to terminate cleanly.",
        ),
      mode: z
        .enum(["editor", "game"])
        .default("game")
        .describe(
          "`game` (default) launches with `-game` for actual gameplay execution (PIE-like). `editor` opens the map in editor mode (use when you need editor-only systems).",
        ),
      timeoutMs: z
        .number()
        .int()
        ...
        .default(300_000)
        .describe(
          "Hard timeout. Defaults to 5 minutes. The process is force-killed if it overruns.",
        ),
      logCategories: z
        .array(z.string())
        .optional()
      ...
      minSeverity: z
        ...
        .default("display")
        .describe(
          "Lowest log severity to include in the captured output. Default `display` keeps gameplay UE_LOG output visible.",
        ),
      pattern: z
        .string()
        .optional()
        .describe(
          "Optional regex (case-insensitive) to filter log message bodies.",
        ),
      maxLogLines: z
        .number()
        .int()
        ...
        .default(150)
        .describe(
          "Max log lines to include in the response. The most recent matches are kept. Set to 0 to skip log embedding (use `read-logs` afterwards).",
        ),
    },
    async ({
      projectPath,
        ...
        if (maxLogLines > 0) {
          if (existsSync(logPath)) {
            ...
            if (afterMtime <= beforeMtime) {
              ...
              lines.push(
                "",
                formatLogEntries(entries, stats, `\`${logPath}\``),
              );
            }
          } else {
            lines.push(
              "",
              "_No log file produced. Use `read-logs` to investigate._",
            );
          }
        }
