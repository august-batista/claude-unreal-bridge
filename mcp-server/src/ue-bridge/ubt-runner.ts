 ...
 */
export async function buildCppTarget(
  project: UEProject,
  options: BuildOptions = {},
): Promise<CppBuildResult> {
  ...
  if (!project.sourcePath || !existsSync(project.sourcePath)) {
    return {
      ...
      warnings: [],
      summary:
        "No Source/ folder — project is Blueprint-only, nothing to build.",
      exitCode: 0,
      durationMs: 0,
    };
  }
  ...
  if (!existsSync(installation.buildScriptPath)) {
    throw new Error(
      `Build script not found at ${installation.buildScriptPath}. ` +
