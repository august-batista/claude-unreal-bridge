 ...
 */
function classifyError(stderr: string, timedOut: boolean, spawnFailed: boolean): PythonErrorType {
  if (spawnFailed)  return "spawn_failed";
  if (timedOut)     return "timeout";

  // Python traceback inside the UE process
  if (/Traceback \(most recent call last\)/i.test(stderr)) return "python_exception";
  if (/LogPython: Error:/i.test(stderr))                   return "python_exception";

  // UE failed to load the project or critical module
  if (/Failed to load.*uproject/i.test(stderr))  return "ue_load_failure";
  if (/Error.*PythonScriptPlugin/i.test(stderr)) return "ue_load_failure";
  if (/couldn't find plugin/i.test(stderr))      return "ue_load_failure";
 ...
 */
function summariseError(stderr: string, errorType: PythonErrorType): string {
  ...
  if (errorType === "ue_load_failure") {
    const lines = stderr.split("\n").filter(l =>
      /error|failed|couldn't/i.test(l) && !/warning/i.test(l)
    );
 ...
 */
export async function runPythonInUE(
  project: UEProject,
  scriptPath: string,
