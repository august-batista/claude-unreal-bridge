 ...
 */
export function formatBlueprintAsMarkdown(
  bp: BlueprintInfo,
  detail: "summary" | "full" | "graph-only" = "full",
): string {
  const lines: string[] = [];
  ...
  if (showMetadata && bp.variables.length > 0) {
    ...
    for (const v of bp.variables) {
      const flags: string[] = [];
      if (v.isEditable) flags.push("Editable");
      if (v.isExposed) flags.push("Exposed");
      ...
      if (v.tooltip) lines.push(`  - ${v.tooltip}`);
    }
  }
...
function collectCustomEventNames(bp: BlueprintInfo): Set<string> {
  ...
  if (execOutputs.length === 1) {
    ...
    for (const pin of execOutputs) {
      ...
      if (pin.connectedTo.length > 0) {
        ...
        const nextNode = nodeMap.get(nextNodeId);
        if (nextNode) {
          if (branchLabel) lines.push(`${prefix}${branchLabel}:`);
          traceExecutionFlow(
            nextNode,
            nodeMap,
            pinLookup,
...
function formatNode(node: BlueprintNode): string {
 ...
 */
export function validateBlueprintJson(data: unknown): data is BlueprintInfo {
  if (!data || typeof data !== "object") return false;
  const obj = data as Record<string, unknown>;
  return (
    typeof obj.assetPath === "string" &&
