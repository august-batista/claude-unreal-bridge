...
function readRegisteredEngines() {
    const out = {};
...
export function findBestInstallation(targetVersion) {
    ...
    if (targetVersion) {
        ...
        const partial = installations.find((i) => i.version.startsWith(majorMinor));
        if (partial)
            return partial;
    }
    // Return newest
...
function tryParseInstallation(enginePath, platform) {
    ...
    if (version === "unknown") {
        try {
            const bv = JSON.parse(readFileSync(join(enginePath, "Engine/Build/Build.version"), "utf-8"));
