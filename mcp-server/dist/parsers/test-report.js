 ...
 */
export function parseTestReport(raw) {
    const r = (raw ?? {});
    const tests = [];
    ...
    for (const t of rawTests) {
        const tt = t;
        const events = [];
        const rawEntries = tt.entries ?? [];
        for (const entry of rawEntries) {
            const e = entry;
            const eventField = e.event ?? e;
            const type = normaliseEventType(eventField.type);
        ...
        const state = normaliseState(tt.state);
        tests.push({
...
function normaliseState(s) {
    ...
    if (v.includes("success") || v === "passed")
        return "Success";
    if (v.includes("fail"))
        return "Fail";
    if (v.includes("skip"))
        return "Skipped";
    if (v.includes("inprocess") || v === "running")
        return "InProcess";
    return "NotRun";
}
function normaliseEventType(t) {
    if (!t)
        return "Info";
 ...
 * everything passed.
 */
export function formatTestReport(report, options = {}) {
    const lines = [];
    ...
    const skipped = report.tests.filter((t) => t.state !== "Success" && t.state !== "Fail");
...
function formatEvent(e) {
    ...
    const usableFile = e.filename && e.filename !== "Unknown" ? e.filename : undefined;
