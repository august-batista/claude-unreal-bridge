 ...
 */
export function parseCppBuildOutput(stdout, stderr, exitCode) {
    const text = stdout + "\n" + stderr;
    ...
    const clangRe = /^(.+?\.(?:cpp|h|hpp|inl|c|cc|mm|cxx)):(\d+):(?:(\d+):)?\s+(error|warning|note|fatal error):\s+(.*)$/i;
    // MSVC:  C:\path\file.cpp(42): error C2065: 'foo': undeclared identifier
    const msvcRe = /^(.+?\.(?:cpp|h|hpp|inl|c|cc|cxx))\((\d+)(?:,(\d+))?\):\s+(error|warning|fatal error)\s+([A-Z]\d+):\s+(.*)$/i;
    ...
    for (const rawLine of lines) {
        ...
        if (/^(Wrote partial receipt|Creating makefile|Setting up bundled DotNet|Running dotnet|Log file:|Using 'git status')/.test(line)) {
            inProseSection = true;
        ...
        if (match) {
            ...
            const severity = sev === "warning" ? "warning" : "error";
            const entry = {
                file: match[1],
        ...
        if (match) {
            ...
            if (/^\s*\d+\s+errors?\b/i.test(match[2]))
                continue;
            pushDedup({
        ...
        if (inProseSection) {
            if (/^Total execution time:/i.test(line))
                continue;
            if (line.length < 8)
                continue;
...
function pushDedup(entry, errors, warnings, seen) {
 ...
 */
export function formatCppBuildResult(result) {
    const lines = [];
...
//# sourceMappingURL=cpp-build-output.js.map