 ...
 */
export function parseCompileOutput(stdout, stderr, exitCode) {
    const text = stdout + "\n" + stderr;
    const lines = text.split(/\r?\n/);
    const errors = [];
    const warnings = [];
    const seen = new Set(); // dedupe (category|severity|message)
    ...
    const COMPILE_CATEGORIES = /^Log(Blueprint|K2Compiler|CompileAllBlueprintsCommandlet|ScriptCompile)\b/;
    ...
    for (const rawLine of lines) {
        ...
        if (!line)
            continue;
        if (!COMPILE_CATEGORIES.test(line))
            continue;
    ...
    const failedCount = totals?.failed ?? (errors.length > 0 ? errors.length : 0);
...
function parseSummary(lines) {
    ...
    for (const rawLine of lines) {
        ...
        if (inFailedNamesBlock) {
            ...
            const nameMatch = line.match(/^LogCompileAllBlueprintsCommandlet:\s+(?:Display:\s+)?(.+)$/);
            if (nameMatch) {
