 ...
 */
export function scanProjectStructure(project) {
    const assetCounts = {};
...
function countAssetsRecursive(dir, counts) {
    let entries;
    try {
...
function buildDirectoryTree(rootDir, maxDepth, currentDepth = 0, prefix = "") {
    if (currentDepth >= maxDepth)
        return "";
    let entries;
    try {
        entries = readdirSync(rootDir).filter((e) => !e.startsWith(".") &&
            e !== "node_modules" &&
            e !== "Intermediate" &&
            e !== "Saved" &&
            e !== "DerivedDataCache" &&
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        if (entry.isDir) {
            const subtree = buildDirectoryTree(join(rootDir, entry.name), maxDepth, currentDepth + 1, prefix + nextPrefix);
            if (subtree)
                lines.push(subtree);
        }
    }
