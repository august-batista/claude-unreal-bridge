...
export function registerCompileBlueprintsTool(server) {
    server.registerTool("compile-blueprints", {
        title: "Compile Blueprints",
        description: "Compile all blueprints in a UE project and report errors/warnings. Uses the CompileAllBlueprints commandlet. " +
        ...
        try {
            ...
            if (extra.signal?.aborted) {
                return {
                    isError: true,
                    content: [
                        { type: "text", text: "Blueprint compilation cancelled." },
                    ],
                };
            }
            return {
