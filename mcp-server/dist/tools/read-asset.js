    ...
    server.tool("read-asset", "Read basic metadata about any Unreal Engine asset. Returns file path, size, type, and related files (.uexp, .ubulk). For blueprint-specific data, use read-blueprint instead.", {
        ...
        try {
            ...
            if (!existsSync(filePath)) {
                ...
                lines.push("", "The asset path may be incorrect. Use `list-blueprints` to find available assets.");
                return {
            ...
            lines.push("");
            lines.push("*For detailed blueprint data (variables, functions, graphs), use the `read-blueprint` tool.*");
            return {
                content: [{ type: "text", text: lines.join("\n") }],
            };
        }
