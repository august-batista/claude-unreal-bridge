...
export function registerCreateAssetTool(server) {
    server.registerTool("create-asset", {
        title: "Create Asset",
        description: "Create a new Blueprint or Widget Blueprint asset (empty shell) in the project's content, " +
        ...
        inputSchema: {
            ...
            parentClass: z
                .string()
                .default("")
                .describe("Parent class: a UE Python name (\"Actor\", \"Pawn\", \"ActorComponent\", \"UserWidget\") or a class path " +
        ...
        try {
            ...
            if (!data.success) {
                return {
                    isError: true,
                    content: [{ type: "text", text: `Create asset failed: ${data.error ?? "unknown error"}` }],
                };
            }
            return {
                content: [
