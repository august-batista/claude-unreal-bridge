...
export function registerEditBlueprintGraphTool(server) {
    server.registerTool("edit-blueprint-graph", {
        ...
        inputSchema: {
            ...
            autoLayout: z
                .boolean()
                .default(false)
                .describe("Auto-arrange the whole graph into a tidy left-to-right flow after applying ops. Recommended when authoring a new graph; avoid on graphs a human has hand-arranged (it repositions every node)."),
        },
        ...
        try {
            ...
            if (extra.signal?.aborted) {
                return { isError: true, content: [{ type: "text", text: "Graph edit cancelled." }] };
            }
            ...
            if (data.error) {
                return {
                    isError: true,
                    content: [{ type: "text", text: `Graph edit failed: ${data.error}` }],
                };
            }
            const structured = {
...
//# sourceMappingURL=edit-blueprint-graph.js.map