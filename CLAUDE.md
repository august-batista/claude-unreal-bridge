...
- `read-blueprint` — Read a specific blueprint's variables, functions, event graph, and components. Returns `structuredContent` with each node's `nodeGuid` and a per-graph `connections` edge list (`{from, fromPin, to, toPin, kind}`, keyed by NodeGuid + pin name) — the same addressing `edit-blueprint-graph` consumes, so read→rewire is mechanical. Graphs/variables parsed from T3D; **components read via the editor's `SubobjectDataSubsystem`** (reliable for Components-panel / SCS components, which don't round-trip through T3D — the old T3D-only path returned `[]` for them, falls back to it if the subsystem is unavailable). (No bridge plugin needed.)
...
- `edit-blueprint-graph` — Add / wire / delete **K2 graph nodes** in one editor session, then compile + save. Batch of ordered ops: `addFunctionNode`, `addSelfFunctionNode` (self/parent funcs), `addCustomEvent`, `addVariableGet`, `addVariableSet`, `addBranch`, `addSequence`, `addMacro` (ForLoop/ForEachLoop/WhileLoop/Gate/DoOnce/FlipFlop), `addCast`, `addMemberVariable`, `connect`, `breakPinLink`, `setPinDefault`, `deleteNode`, `moveNode`, `retargetNode` — enough for real logic (variables + branches + loops + casts + math/function calls). **Edits the existing graph in place** (not a rewrite): nodes spawned in the batch are referenced by a local `id`, existing nodes by GUID (from `read-blueprint`), so you can surgically wire/unwire (`connect`/`breakPinLink`), set pin defaults, move, delete, or reconfigure individual nodes (`retargetNode` retargets a function call / cast / variable or renames a custom event). Create a variable with `addMemberVariable` before get/set nodes reference it — it takes the six primitives plus `struct`/`object`/`class` types and `array`/`set`/`map` containers (via `varType` + `typePath` + `container`), the complex types built through `BlueprintEditorLibrary`'s pin-type factory (pure Python; only the primitive path uses the bridge). Pass `autoLayout: true` to tidy the graph (exec backbone on a pin-aligned rail; data/condition feeders packed into the gap before their consumer; disconnected events parked); `operations` may be empty for a layout-only re-arrange. Returns structured per-op results + node GUIDs + positions + the live post-edit `connections` edge list (via `ListGraphConnections`). **Requires the `ClaudeUnrealBridge` plugin enabled and its editor target built** in the project (it ships the `UClaudeBPGraphLibrary` C++ wrapping `FGraphNodeCreator` / schema `TryCreateConnection` / `UEdGraph::RemoveNode` / `FKismetEditorUtilities::CompileBlueprint` + `AutoLayoutGraph` — all UE 5.7+, no 5.8 needed). Driver: `python-scripts/edit_blueprint_graph.py`. Regression: `cd mcp-server && npm run test:graph` (gated; needs UE 5.7 + a built `SandboxEditor`, skips otherwise).
- `create-asset` — Create a new **Blueprint or Widget Blueprint asset** (empty shell) with a chosen parent class, via the editor's `AssetTools` + a Blueprint/Widget factory (the `FKismetEditorUtilities::CreateBlueprint` path Epic's own tools use). **Pure Python — no bridge plugin needed.** Returns the new asset path; follow with `edit-blueprint-graph` (logic + variables), `add-component`, or `design-widget` to populate it. Driver: `python-scripts/create_asset.py`.
- `add-component` — Add a component to a Blueprint's component tree (the Components panel), attached under an existing component by name or at the root, then compile + save. Uses the editor's `SubobjectDataSubsystem` (`AddNewSubobject` — the same API the Components panel uses). **Pure Python — no bridge plugin needed.** Driver: `python-scripts/add_component.py`.
- `design-widget` — Design a **Widget Blueprint's UMG layout**: ordered ops `addWidget` (panels/buttons/text/images into the tree, parented by name; first empty-parent widget becomes the root) and `setCanvasSlot` (position/size a widget under a CanvasPanel), then compile + save. WidgetTree population **requires the bridge** (`UClaudeBPGraphLibrary::AddWidgetToTree`/`SetCanvasSlotLayout`/`ListWidgets`, cribbed from UE 5.8's `UMGToolSet` — `UWidgetTree::ConstructWidget` + panel `AddChild`, all UE 5.7) because `UWidgetTree` isn't exposed to Python; the bridge editor module gains `UMG`+`UMGEditor` deps. Create the asset first with `create-asset` (assetKind: widget). Driver: `python-scripts/design_widget.py`. Regression for all four authoring tools: `cd mcp-server && npm run test:assets` (gated like `test:graph`).
