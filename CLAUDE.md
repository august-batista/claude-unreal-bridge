...
- `read-blueprint` — Read a specific blueprint's variables, functions, event graph. Returns `structuredContent` with each node's `nodeGuid` and a per-graph `connections` edge list (`{from, fromPin, to, toPin, kind}`, keyed by NodeGuid + pin name) — the same addressing `edit-blueprint-graph` consumes, so read→rewire is mechanical. (No bridge plugin needed; parsed from T3D.)
